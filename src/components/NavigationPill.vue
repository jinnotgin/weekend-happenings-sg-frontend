...
<template>
	<div class="nav-pill lg:hidden fixed bottom-4 left-1/2 z-40 -translate-x-1/2 select-none" :class="{
		...
		<GlassSurface class="nav-pill__surface text-white shadow-2xl rounded-full" width="auto" height="auto"
			:border-radius="999" :border-width="0.16" :brightness="62" :opacity="0.85" :blur="18" :displace="1.1"
			:background-opacity="0.1" :saturation="1.6" :distortion-scale="-120" :red-offset="2" :green-offset="12"
			:blue-offset="24" x-channel="R" y-channel="G" mix-blend-mode="screen" content-class="nav-pill__content">
			<div class="nav-pill__body flex items-center gap-2">
				<button type="button"
					...
					@mousedown.prevent="handlePressStart('up')" @touchstart="handlePressStart('up')"
					@mouseup.prevent="handlePressEnd" @mouseleave="handlePressCancel" @touchend="handlePressEnd"
					@touchcancel="handlePressCancel" :aria-pressed="pressedDirection === 'up'"
					aria-label="Scroll to previous event">
					<VueFeather type="chevron-up" class="h-4 w-4" />
				</button>
				<span class="nav-pill__count font-sans text-[11px] font-semibold tracking-[0.35em]"
				...
				<button type="button"
					...
					@mousedown.prevent="handlePressStart('down')" @touchstart="handlePressStart('down')"
					@mouseup.prevent="handlePressEnd" @mouseleave="handlePressCancel" @touchend="handlePressEnd"
					@touchcancel="handlePressCancel" :aria-pressed="pressedDirection === 'down'"
