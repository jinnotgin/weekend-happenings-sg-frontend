...
<template>
	<main class="flex flex-col gap-8 px-0">
		<div class="mx-auto flex w-full scale-125 flex-col items-center bg-white/85 backdrop-blur pb-14 sm:pb-28"
			v-if="events.fetchEventsStatus !== 'success'">
			<!-- hack: using scale-125 to combat the popping issue-->
			<div class="flex w-full flex-col items-center gap-6" v-show="events.fetchEventsStatus === 'idle' ||
				...
				<CurvedLoop marquee-text="p ✦ Buzz ✦ Scoo" text-class="font-sans uppercase tracking-[0.35em]" :speed="3.5"
					:curve-amount="400" :interactive="false" />
			</div>
			<div v-show="events.fetchEventsStatus === 'error'"
		...
		<div v-show="events.fetchEventsStatus === 'success'" class="flex flex-col gap-6 pb-6 lg:gap-8">
			...
			<div v-if="isAwaitingLocationResults"
			...
			<div v-show="shouldShowEmptyState"
				class="mx-auto w-full max-w-6xl rounded-[32px] border-4 border-black bg-white/80 px-4 py-8 text-center font-sans text-sm shadow-[8px_8px_0_#1f1b2c] backdrop-blur sm:px-8 sm:py-12 sm:text-lg sm:shadow-[16px_16px_0_#1f1b2c]">
			...
			<ul class="flex flex-col gap-8" v-show="filteredEvents.length > 0" ref="eventListRef">
				<li v-for="(event, index) in filteredEvents" :key="index" class="transition-transform duration-200"
					data-event-card :data-event-index="index">
					<div
						...
						<div class="relative z-10 flex flex-col-reverse gap-6 lg:flex-row lg:items-start lg:gap-10">
							<div class="flex flex-1 flex-col gap-5">
								<div class="flex items-start gap-3 sm:gap-4">
									...
									<div class="ml-auto flex items-start gap-2">
										<a :href="event.link" target="_blank" rel="noopener" aria-label="Open event in a new tab"
										...
										<VMenu class="inline-flex">
											...
											<template #popper>
												<div
													...
													<a v-for="({ title, url }, index) in events.getSourcesData(
														event.source
													)" :key="index"
														:href="url" target="_blank" class="mt-2 block text-left underline-offset-4 hover:underline">
														{{ index
															+ 1 }}.
														{{ title
														}}
								...
								<div class="grid items-start gap-3 sm:grid-cols-3 sm:gap-6">
									<div
										class="rounded-2xl border-2 border-dashed border-[#1f1b2c]/40 bg-[#ffe066]/30 px-3 py-2.5 sm:px-4 sm:py-3">
									...
									<div
										class="rounded-2xl border-2 border-dashed border-[#1f1b2c]/40 bg-[#8ec5ff]/20 px-3 py-2.5 sm:px-4 sm:py-3">
										<p class="font-sans text-xs uppercase tracking-[0.3em] text-[#f15a24]">
											Location
										</p>
										<div class="mt-2 flex flex-col gap-2">
											<div class="flex items-start gap-2">
												<div class="flex-1 flex flex-col gap-1 lg:gap-1.5">
													...
													<p v-if="event.locationSummary.details" class="font-sans text-xs text-[#1f1b2c]/70">
														{{
															event.locationSummary.details
														}}
													</p>
												</div>
												<a v-if="event.locationSummary.mapUrl" :href="event.locationSummary.mapUrl" target="_blank"
													rel="noopener"
											...
											<div v-if="event.locationSummary.additionalLocations" class="flex flex-col gap-2">
												<button type="button"
													...
													@click="toggleLocationsExpanded(index)" :aria-expanded="isLocationsExpanded(index)"
													:aria-controls="`event-location-list-${index}`">
													<vue-feather :type="isLocationsExpanded(index) ? 'chevron-up' : 'chevron-down'"
														class="h-3.5 w-3.5" />
													<span v-if="!isLocationsExpanded(index)">
														...
														more
														location<span v-if="event.locationSummary.additionalLocations > 1">s</span>
													</span>
													<span v-else>Hide
														extra
														locations</span>
												</button>
												<ul v-if="isLocationsExpanded(index)" :id="`event-location-list-${index}`"
													class="space-y-2 border-t border-dashed border-[#1f1b2c]/20 pt-2">
													<li v-for="(location, locationIndex) in event.locationSummary.otherLocations"
														:key="locationIndex" class="flex items-start gap-2">
														...
														<div class="flex flex-1 flex-col gap-1">
															<div class="flex items-start gap-2">
																...
																<a v-if="location.mapUrl" :href="location.mapUrl" target="_blank" rel="noopener"
															...
															<p v-if="location.details" class="font-sans text-xs text-[#1f1b2c]/70">
																{{
																	location.details
																}}
															</p>
									...
									<div
										class="rounded-2xl border-2 border-dashed border-[#1f1b2c]/40 bg-[#ff8ba7]/20 px-3 py-2.5 sm:px-4 sm:py-3">
										<p class="font-sans text-xs uppercase tracking-[0.3em] text-[#f15a24]">
											Type
										</p>
										<div class="mt-2 flex flex-wrap gap-2">
											<span v-for="(category, index) in event.category" :key="index"
							...
							<div class="lg:flex-none lg:w-80">
								<a :href="event.link" target="_blank"
									...
									<img :src="event.thumbnail" :alt="event.title" class="aspect-video w-full object-cover"
										loading="lazy" />
								</a>
							</div>
						</div>
			...
			<NavigationPill v-if="shouldShowNavigator" :bounce-state="bounceState" :pressed-direction="pressedDirection"
				:hold-effect-direction="holdEffectDirection" :current-display-index="currentDisplayIndex"
				:total-events="totalEvents" @navigator-click="handleNavigatorClick" @press-start="handlePressStart"
